...
Public Class Form9
    ...
    Private Async Sub AddData_Click(sender As Object, e As EventArgs) Handles AddData.Click
        ...
        Try
            AddDataToExcel(name, surname, registrationTime, registrationDate)
            ...
            If Await IsInternetAvailableAsync() Then
                Try
                    AddDataToGoogleSheets(name, surname, registrationTime, registrationDate)
                    ShowStatusMessage("Data saved online successfully!", MessageBoxIcon.Information)
                Catch ex As Exception
                    HandleOfflineSave(name, surname, registrationTime, registrationDate)
                End Try
            Else
                HandleOfflineSave(name, surname, registrationTime, registrationDate)
            End If
    ...
    Private Function ValidateInputs() As Boolean
        If String.IsNullOrWhiteSpace(TBox_Name.Text) OrElse
           String.IsNullOrWhiteSpace(TBox_Surname.Text) Then
    ...
    Private Sub HandleOfflineSave(name As String, surname As String,
                                registrationTime As String, registrationDate As String)
        SavePendingOnlineData(name, surname, registrationTime, registrationDate)
    ...
    Private Sub SavePendingOnlineData(name As String, surname As String,
                                    registrationTime As String, registrationDate As String)
        Try
            Dim dataLine As String = $"{name},{surname},{registrationTime},{registrationDate}"
            ...
            For Each line In pendingLines
                Try
                    ...
                    If fields.Length = 5 Then
                        AddDataToGoogleSheets(fields(0), fields(1), fields(2), fields(3))
                        syncedCount += 1
                    End If
                Catch ex As Exception
    ...
    Private Sub AddDataToGoogleSheets(name As String, surname As String, registrationTime As String, registrationDate As String)
        Try
            ...
            .Values = New List(Of IList(Of Object)) From {New List(Of Object) From {name, surname, registrationTime, registrationDate}}
        }
    ...
    Private Sub AddDataToExcel(name As String, surname As String, registrationTime As String, registrationDate As String)
        Try
